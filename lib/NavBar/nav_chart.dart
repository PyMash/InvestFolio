...
class NavChartPage extends StatefulWidget {
  NavChartPage({
    Key? key,
  });

  @override
  State<NavChartPage> createState() => _NavChartPageState();
}
...
class _NavChartPageState extends State<NavChartPage> {
                  ...
                  : LineChart(LineChartData(
                      ...
                      lineBarsData: [
                        ...
                        LineChartBarData(
                          ...
                          spots: dataPoints.asMap().entries.map((entry) {
                            return FlSpot(
                              entry.key.toDouble(),
                              entry.value.roiAmount.toDouble().roundToDouble(),
                            );
                          }).toList(),
                          isCurved: true,
                          gradient: LinearGradient(colors: [
                      ...
                      gridData: FlGridData(
                        ...
                        getDrawingHorizontalLine: (value) {
                          return FlLine(
                              color: Colors.green.shade800, strokeWidth: 0.8);
                        },
                      ),
                          ...
                          sideTitles: SideTitles(
                            ...
                            getTitlesWidget: (value, meta) {
                              if (value >= 0 && value < dataPoints.length) {
                                ...
                                final formattedRoi = dataPoint.roiAmount > 999
                                    ? '${(dataPoint.roiAmount / 1000).toStringAsFixed(1)}K'
                                    : '${dataPoint.roiAmount.toStringAsFixed(0)}';
  ...
  String formatTooltipText(DataPoint dataPoint) {
    final formattedInvestment = formatAmount(dataPoint.investmentAmount);
    final formattedRoi = formatAmount(dataPoint.roiAmount);
    final date = DateTime(dataPoint.year, dataPoint.month);
